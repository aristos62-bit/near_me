...
### P0.1 — `_deleteChatForEveryone`: One-by-one message delete + κίνδυνος crash (batch 500 limit)

| Αρχείο | Γραμμές | Κόστος τώρα |
|--------|---------|-------------|
| `chat_repository_delete.dart` | 88-137 | 1 read + N writes (όσα messages) |

**Πρόβλημα (2 issues):**

1. **One-by-one delete:** Διαγράφει messages ένα-ένα σε loop (γραμμές 104-112). Για chat με 500 messages = **500 ξεχωριστές writes** αντί 1 batch.
2. **Batch 500 limit:** Αν απλά αλλάξουμε σε `firestore.batch()` χωρίς pagination, θα σκάσει όταν το chat έχει >500 messages (Firestore hard limit: 500 operations per batch).

```dart
// ❌ ΤΩΡΑ (chat_repository_delete.dart:104-112) — one-by-one delete:
for (final doc in messages.docs) {
  await firestore.collection('chats').doc(chatId)
      .collection('messages').doc(doc.id).delete();
}

// ❌ ΛΑΘΟΣ ΛΥΣΗ — αν βάλουμε batch χωρίς pagination:
final batch = firestore.batch();
for (final doc in messages.docs) {          // ⚠️ Αν >500 docs → crash!
  batch.delete(doc.reference);
}
await batch.commit();
```

**Λύση — Pagination loop με batches των 500:**

```dart
// ✅ ΣΩΣΤΗ ΛΥΣΗ — pagination loop, max 500 ops/batch:
const batchSize = 500;
...
while (messages.docs.isNotEmpty) {
  ...
  if (messages.docs.length < batchSize) break; // τελείωσαν

  // Επόμενο page
  final lastDoc = messages.docs.last;
  final nextPage = await firestore
      ...
      .get();
  messages.docs.clear();
  messages.docs.addAll(nextPage.docs);
}
```

**Τι αλλάζει:**
- Από N writes (1 per message) σε ~ceil(N/500) batches
- Προστασία από crash όταν chat >500 messages
- Δεν χάνεται λειτουργικότητα
...
### P0.4 — `clearMessages`: Unbounded `.get()` + BUG σε production (crash όταν >500 messages)

| Αρχείο | Γραμμές | Κόστος τώρα |
|--------|---------|-------------|
| `chat_repository_impl.dart` | 635-643 | Διαβάζει ΟΛΑ τα messages και κάνει 1 batch |

**Πρόβλημα (2 issues):**

1. **Unbounded read:** `.get()` χωρίς `.limit()` — φορτώνει ΟΛΑ τα messages (γραμμή 635-637).
2. **CRASH BUG:** Το `clearMessages` ΗΔΗ κάνει batch.delete (γραμμές 639-642) **αλλά ΧΩΡΙΣ pagination**. Αν κάποιο chat έχει >500 messages, το batch θα πετάξει `Firestore exception: maximum 500 operations per batch` — **crash στο production**.

```dart
// ❌ ΤΩΡΑ (chat_repository_impl.dart:635-643) — unbounded read + no pagination:
final messages = await firestore
    .collection('chats').doc(chatId).collection('messages')
    .get();  // ← no .limit()!
...
final batch = firestore.batch();
for (final doc in messages.docs) {
  batch.delete(doc.reference);  // ⚠️ Αν >500 docs → crash!
}
await batch.commit();
```

**Λύση — Pagination loop με batches των 500 (όπως P0.1):**

```dart
// ✅ ΣΩΣΤΗ ΛΥΣΗ — pagination loop:
const batchSize = 500;
bool hasMore = true;
...
**Τι αλλάζει:**
- Από 1 crash-prone batch → multiple safe batches των 500
- Προστασία από crash σε chat με >500 messages (παραγωγή)
- Ελάχιστη αύξηση writes (αντί 1 batch, γίνονται ~N/500 batches)

**Warning:** Το `clearMessages` καλεί CURRENT USER's messages μόνο, αλλά αν υπάρχουν 600 messages → crash σήμερα. **Fix αυτό πρώτο από όλα.**
...
|-----------|---------|--------|--------|
| `deleteChatForEveryone` | One-by-one delete ❌ | Batch delete ✅ | **P0.1** |
| `clearMessages` | Batch ✅ (αλλά no limit στο read) | Batch + Limit ✅ | **P0.4** |
...
### Φάση Α — Κρίσιμα (P0 — 2 remaining)
| Σειρά | Θέμα | Impact | Εκτίμηση |
|:-----:|------|:------:|:--------:|
| **1** | **P0.4** — `clearMessages` pagination loop (BUG: crash >500 msgs) | **Crash fix** + Κόστος | 30 λεπτά |
| **2** | **P0.1** — `_deleteChatForEveryone` pagination + batch | Κόστος + Crash fix | 30 λεπτά |

> **Σημείωση:** Το P0.4 μπαίνει πρώτο γιατί είναι **ενεργό bug στο production** — το `clearMessages` μπορεί να crashάρει ανά πάσα στιγμή σε chat με >500 μηνύματα.
