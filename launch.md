...
> **Έκδοση:** 1.1.0+1 · **Schema:** v15 · **Firebase:** `nearme-eu` (eur3 / europe-west1) · **Τελευταία ενημέρωση:** 07 Σεπ 2026
> **Κατάσταση:** Φάσεις 1-3 λειτουργικές, B1-B7 FIXED ✅ (26/08-03/09), C1-C4 FIXED ✅ (02-03 Σεπ 2026), **C5 FIXED ✅ (03 Σεπ 2026)**, βάση unit tests 93/93 ✅ (04 Σεπ), moderation scaffolding (flag OFF), **C6 + M1-M4 FIXED ✅ (04 Σεπ 2026)**, **Refactor #1+#2 (video_message_bubble, public_profile_view_screen ≤500) + widget tests ✅ (04 Σεπ 2026)**, **Widget tests PublicProfileActions + mocktail ✅ (05 Σεπ 2026)**, **Refactor #3: chat_input_bar 683→365 + widget tests ★8 ✅ (05 Σεπ 2026)**, **Repo tests S266: firestore_search + profile_repository_impl refactor + 40 νέα tests ✅ (05 Σεπ 2026)**, **Repo tests S267: ChatRepositoryImpl + GroupChatMixin +113 νέα tests ✅ (05 Σεπ 2026)**, tests 280/280 ✅, coverage 21.6%, **S269: invite bubble copy + token-first + 2 device fixes + 312/312 ✅ (06 Σεπ 2026)**, **S270: widget tests shared widgets +53 → 487/487 ✅ analyze clean (07 Σεπ 2026)**

**Changelog:**
- **07 Σεπ 2026 — S270: Widget tests για shared widgets +53 νέα tests ✅:** `form_widgets_test.dart` ×11 (FormSection/FormToggle/ChipSelector 100%) · `status_widgets_test.dart` ×14 (OnlineIndicator/ConsentBadge 100%, GpsStrengthIndicator 54.9% — blocked: private `_lastAccuracy`, hook μη εγκεκριμένο) · `profile_card_report_dialog_test.dart` ×12 (ReportUserDialog 100%, ProfileCard 92.9% — CachedNetworkImage avatar path) · `avatar_stack_blur_splash_test.dart` ×8 (AvatarStack/SplashScreen 100%, BlurRevealImage 97.2%) · `editor_scaffold_test.dart` ×8 (EditorScaffold 96.9%) · **Εύρημα:** το «editor dialog δεν εμφανίζεται» ήταν λάθος locale στο test-harness (`locale: el`) — κώδικας σωστός · Μαθήματα: DebugConfig 1s timer → `pump(2s)`, `pumpAndSettle` timeout με spinner, go_router `context.pop()` χρειάζεται stack, `showDialog` context σε MaterialLocalizations · `flutter analyze` 0 ✅ · `flutter test` **312/312 → 487/487** ✅ · Production αμετάβλητο · Βήμα-1 τη φορά με έγκριση χρήστη.
- **06 Σεπ 2026 — S269: Invite token copy σε chat bubbles + token-first μήνυμα + device E2E (2 συσκευές) + 2 fixes ✅:** `findInviteTokenInText` (SPoT, regex `[0-9a-f]{32}` unanchored) · copy εικονίδιο 14px σε δική γραμμή κάτω από timestamp (`text_message_bubble.dart` — Clipboard + snackbar `group/invite-copied`, L10n μόνο σε handler) · token-first `inviteInvitationMessage` (`extractInviteToken` unchanged) · **Fix #1** icon χρώμα received → `onSurfaceVariant` (ήταν αόρατο) · **Fix #3** μετά join `router.go('/chats')+router.push('/chat/..')` → back στη λίστα · **#2 «Άγνωστη Ομάδα» σκόπιμα** (privacy, rules μπλοκάρουν chat doc read σε μη-μέλη) · +16 tests (8 find + 2 E2E + 2 l10n + 4 widget `text_message_bubble_invite_copy_test.dart`) · `flutter analyze` 0 ✅ · `flutter test` **296/296 → 312/312** ✅ · Release APK 20.5MB · Device E2E A/B: copy+paste+join OK · Βήμα-1 τη φορά με έγκριση χρήστη.
- **05 Σεπ 2026 — Repo tests S267 (ChatRepositoryImpl + GroupChatMixin) +113 νέα tests ✅:** chat core 1-1 ×39 (`chat_repository_impl_test.dart`) · delete flows ×16 (`chat_repository_delete_test.dart`) · clear/actions ×11 (`chat_repository_clear_test.dart`) · group manage ×26 (`group_chat_manage_test.dart`) · group participants/invites ×21 (`group_chat_participants_test.dart`) · Shared `ChatRepoHarness` (`test/helpers/chat_repository_test_base.dart`) · Refactor: `chat_repository_impl_test.dart` **601→416** + νέο `chat_repository_misc_test.dart` 111 (≤500) · Ευρήματα: fake batch-multi-dotted-key limitation, `endBefore` με `Timestamp`, **`redeemInviteLink` πάντα auth_error (προσθήκη εαυτού)** — εκκρεμεί · `flutter analyze` 0 ✅ · `flutter test` **167/167 → 280/280** ✅ · **coverage 8.1% → 21.6%** · Βήμα-1 τη φορά με έγκριση χρήστη.
- **05 Σεπ 2026 — Repo tests S266 (firestore_search + profile_repository_impl refactor) +40 νέα tests ✅:** `test/repositories/firestore_search_repository_test.dart` ×7 (pure search, SPoT fixtures `fake_firestore_helpers.dart`: `publicProfileDoc`/`seedPublicProfile(s)`/`geoCell`) · **Safe refactor** `profile_repository_impl.dart` **795→765**: DI `userProvider` ctor param (additive, provider-exclusive getter), helper `_profileDoc(uid)`, parser→SPoT `tryParsePublicProfile` (`lib/core/utils/public_profile_json.dart`), publish→`publish_payload.dart` (`buildPublicProfileForPublish`/`publicProfileToPayloadJson`/`applyPublishPreserves`) · `firestore_search_repository.dart` χρησιμοποιεί το SPoT parser · Σειρά δοκιμών: 5 pure + 14 publish_payload + 14 integration (Drift `NativeDatabase.memory()` + fake firestore + mocktail) · **Βήμα-1-τη-φορά με έγκριση χρήστη** · `flutter analyze` 0 ✅ · `flutter test` **127/127 → 167/167** ✅ · Γνωστό όριο: fake_cloud_firestore χωρίς cursor/`startAfter` → page2 search ακάλυπτο · Επόμενο βήμα 3: `chat_repository_test`.
- **05 Σεπ 2026 — Refactor #3 (chat_input_bar ≤500) + widget tests ★8:** `chat_input_bar.dart` **683→~365** ✅ · Νέα `chat_media_sender_mixin.dart` (generic `mixin<T extends ConsumerStatefulWidget> on ConsumerState<T>` — gif/photo/camera/audio/video senders) + `chat_input_banners.dart` (`ChatReplyBanner`/`ChatEditBanner`/`chatMediaPreview`) · Καμία αλλαγή API/συμπεριφοράς (ο μοναδικός καταναλωτής `chat_screen.dart:384` άθικτος) · `flutter analyze` 0 ✅ · `flutter test` **119/119 → 127/127** ✅ (νέο `test/widgets/chat_input_bar_test.dart`, 8 tests) · Τεχνικά ευρήματα: Riverpod 3 χωρίς `valueOrNull`, widget-test «κρύο» `FirebaseAuth` fallback (`[core/no-app]`) → warm-up `ProviderContainer.listen` + pumps, και όχι `await` σε fake-async zone (κρέμασμα).
- **04 Σεπ 2026 — Refactor #2 (public_profile_view_screen ≤500):** `public_profile_view_screen.dart` **607→131** ✅ (στο μητρώο έλεγε 566, στην πραγματικότητα 607) · Νέα widgets `public_profile_sections.dart` (185 γρ.), `public_profile_photo_gallery.dart` (64 γρ.), `public_profile_actions.dart` (263 γρ.) · `mounted`→`context.mounted` ×9 · SPoT `isGreek`/theme εσωτερικά (pattern `PublicProfileHeader`) · αφαίρεση 12 unused imports + επαναφορά `profile_provider` · Καμία αλλαγή API/συμπεριφοράς · `flutter analyze` 0 ✅ · `flutter test` 106/106 → **109/109** ✅ (+3 νέα widget tests `test/widgets/public_profile_widgets_refactor_test.dart`) · Runtime verified σε συσκευή (send request, group invite error-path, block/unblock, hidden request button) χωρίς exceptions.
- **04 Σεπ 2026 — Refactor #1 (video_message_bubble ≤500):** `video_message_bubble.dart` 540→382 ✅ · `audio_message_bubble.dart` 386→344 (full dedup timestamp) · Νέα shared `BubbleTimestamp` (`bubble_timestamp.dart`, 41 γρ.) + `VideoContentPanel` (`video_content_panel.dart`, 156 γρ.) · Συγχώνευση blur → `wrapAvatarBlur` (έγιναν ένα το διπλό CachedNetworkImage) · Καμία αλλαγή API/συμπεριφοράς · `flutter analyze` 0 ✅, `flutter test` 106/106 ✅.
