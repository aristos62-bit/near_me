...
> **Κατάσταση:** Φάσεις 1-3 λειτουργικές, B1-B7 FIXED ✅ (26/08-03/09), C1-C4 FIXED ✅ (02-03 Σεπ 2026), **C5 FIXED ✅ (03 Σεπ 2026)**, βάση unit tests 93/93 ✅ (04 Σεπ), moderation scaffolding (flag OFF), **C6 + M1-M4 FIXED ✅ (04 Σεπ 2026)**, **Refactor #1+#2 (video_message_bubble, public_profile_view_screen ≤500) + widget tests ✅ (04 Σεπ 2026)**, **Widget tests PublicProfileActions + mocktail ✅ (05 Σεπ 2026)**, tests 119/119 ✅

**Changelog:**
- **05 Σεπ 2026 — Widget tests PublicProfileActions + mocktail:** Νέο `test/widgets/public_profile_actions_test.dart` (**10 tests**) + `mocktail ^1.0.4` (pubspec.yaml:86, dev dep) ✅ · Καλύπτει όλη τη λογική των 4 κουμπιών (canComm/isSelf/no-comm/block-toggle/group-sheet/report) με mocks (`User`/`BlockRepository`/`ReportRepository`) + `_FakeChatActionsNotifier` · 3 πραγματικές ρίζες failures που βγήκαν από το τρέξιμο: `DebugConfig` 1s timer (λύση `pumpWidget(SizedBox)`+`pump(2s)`), stream microtask timing (`pumpAndSettle`), «κρύο» `chatsProvider` (`Consumer`+`ref.watch`) · `flutter analyze` 0 ✅ · `flutter test` **109/109 → 119/119** ✅ · Κανένα runtime αρχείο δεν άλλαξε.
