...
final participantUidsProvider = Provider.autoDispose.family<List<String>, String>((ref, chatId) {
  ...
  ref.onDispose(() {
    DebugConfig.log(DebugConfig.providerDispose, 'participantUidsProvider disposed (derived) for chat: $chatId');
  });
...
class ChatActionsNotifier extends Notifier<ChatActionState> {
  ...
  Future<bool> sendMediaMessage(String chatId, {
    ...
    state = const ChatActionState(status: ChatActionStatus.loading);
    try {
      await _chatRepo.sendMediaMessage(chatId, content: content, type: type, replyTo: replyTo);
