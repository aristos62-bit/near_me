...
class _GroupSettingsScreenState extends ConsumerState<GroupSettingsScreen> {
  ...
  Future<void> _saveMessageExpiry(String value) async {
    final greek = L10n.isGreek(context);
    if (!await ConnectivityGuard.ensure(context)) return;
    setState(() => _isSavingExpiry = true);
    try {
      await ref.read(chatActionsProvider.notifier).updateMessageExpiry(widget.chatId, value);
      if (!mounted) return;
      setState(() => _isSavingExpiry = false);
      AppMessenger.showSuccess(context, ErrorMessages.get('chat/message-expiry-updated', greek));
    } catch (e, s) {
      DebugConfig.error('GroupSettings: saveMessageExpiry failed', data: e, exception: s);
      if (!mounted) return;
      setState(() => _isSavingExpiry = false);
      AppMessenger.showError(context, ErrorMessages.get('chat/message-expiry-update-failed', greek));
    }
  }

  DropdownMenuItem<String> _expiryDropdownItem(String value, String label) {
  ...
  Widget build(BuildContext context) {
    ...
    final chatDoc = ref.watch(chatDocProvider(widget.chatId));
      ...
      body: chatDoc.isLoading
          ...
          : ListView(
              ...
              children: [
                ...
                if (FeatureFlags.messageExpiryEnabled && isCreator) ...[
                  const SizedBox(height: 16),
                  _sectionHeader(context, greek ? 'Αυτόματη Διαγραφή Μηνυμάτων' : 'Message Auto-Delete'),
                  Card(
                    child: Padding(
                      padding: const EdgeInsets.all(16),
                      child: Row(children: [
                        Expanded(
                          child: DropdownButtonFormField<String>(
                            key: ValueKey(chatData?['messageExpiry'] ?? 'off'),
                            initialValue: chatData?['messageExpiry'] as String? ?? 'off',
                            decoration: InputDecoration(
