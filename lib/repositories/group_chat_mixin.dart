...
mixin GroupChatMixin {
  ...
  Future<void> _syncGroupChatToCache(String chatId, Map<String, dynamic> data) async {
    ...
    DebugConfig.log(DebugConfig.repositoryResult,
        '_syncGroupChatToCache: unread count=$unreadCount chat=$chatId');
  ...
  Future<void> updateMessageExpiry(String chatId, String value) async {
    ...
    if (roles[uid] != 'creator') {
      throw AppException(
        code: 'chat/message-expiry-creator-only',
        message: 'Μόνο ο δημιουργός μπορεί να αλλάξει αυτήν τη ρύθμιση / Only the creator can change this setting',
      );
    }
    ...
    try {
      ...
      DebugConfig.log(DebugConfig.chatMessageExpiry,
          'updateMessageExpiry: done chat=$chatId value=$value');
    } catch (e, s) {
      if (e is AppException) rethrow;
      DebugConfig.error('updateMessageExpiry failed', data: e, exception: s);
      throw AppException.firestore('update_expiry',
          'Αποτυχία αλλαγής αυτόματης διαγραφής / Failed to update expiry');
    }
  }
