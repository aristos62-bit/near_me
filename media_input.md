...
> **Κατάσταση:** Revised after codebase re-audit + security review
...
|---------|:---------:|-----------------------|
| `emoji_picker_flutter` στο pubspec.yaml | ❌ **ΔΕΝ υπάρχει** | `flutter pub add emoji_picker_flutter` |
| `_ChatInputBar` (private) → extraction σε νέο widget | ⚠️ Πρέπει να γίνει public | Μετονομασία σε `ChatInputBar`, νέο αρχείο |
| `chat_screen.dart` γραμμή 266: `_ChatInputBar(chatId:...)` | ⚠️ Πρέπει να αλλάξει | Μετά extraction → `ChatInputBar(chatId:...)` |
| `chat_screen.dart` γραμμή 374 — μετά extraction θα μείνει ~270γρ | ✅ <500 γραμμές | Καμία |
| Feature flag για emoji | **Δεν χρειάζεται** | Δεν αλλάζει backend, message format, ούτε data model |
| Firestore / Storage / Cloud Functions | **Καμία αλλαγή** | Emoji picker = client-side UI μόνο |
| `flutter analyze` πριν ξεκινήσουμε | ✅ Must be clean | Αν όχι, fix πριν |

### 5.2 Εμπλεκόμενα Αρχεία

| Αρχείο | Αλλαγή | Γραμμές |
|--------|--------|:-------:|
| `pubspec.yaml` | +`emoji_picker_flutter: ^4.0.0` | +1 |
| **ΝΕΟ:** `lib/features/chat/widgets/chat_input_bar.dart` | Extracted `ChatInputBar` από `chat_screen.dart` + emoji logic | ~180 |
| `lib/features/chat/screens/chat_screen.dart` | Αφαίρεση `_ChatInputBar` class, import νέου widget | ~270 (από 374) |

### 5.3 Κανόνες Εφαρμογής (από codebase)

#### 5.3.1 Multi-language (el/en)

```dart
// Στο build(), όπως ήδη κάνει το _ChatInputBar:
final greek = L10n.isGreek(context);

// Hint text (υπάρχον pattern):
widget.isGroupChat
    ? (greek ? 'Γράψε στην ομάδα...' : 'Type to group...')
    : (greek ? 'Γράψε ένα μήνυμα...' : 'Type a message...');

// Tooltip για emoji button (universal — ίδιο και στις 2 γλώσσες):
greek ? 'Emoji' : 'Emoji'

// Error messages (υπάρχον pattern):
ErrorMessages.get('chat/send-failed', greek);
```

#### 5.3.2 Responsive (LayoutBuilder + ResponsiveUtils)

Το υπάρχον `_ChatInputBar` ήδη χρησιμοποιεί LayoutBuilder με `ResponsiveUtils.resolveWidth()` και `paddingValueFromWidth()`. Το emoji picker (250px fixed) προστίθεται **μέσα** στο ίδιο LayoutBuilder — δεν το σπάει:
...
#### 5.3.3 Debug logging (DebugConfig)

```dart
// Σε init/dispose:
DebugConfig.log(DebugConfig.uiInteraction, 'ChatInputBar init: ${widget.chatId}');
DebugConfig.log(DebugConfig.uiInteraction, 'ChatInputBar dispose: ${widget.chatId}');

// Σε emoji toggle:
DebugConfig.log(DebugConfig.uiInteraction, 'ChatInputBar: emoji picker shown/hidden');

// Σε build:
DebugConfig.log(DebugConfig.uiInteraction, 'ChatInputBar build: canComm=$canComm emojiVisible=$_emojiPickerVisible');
```

**Σημαντικό:** Χρησιμοποιείται το **υπάρχον** `DebugConfig.uiInteraction` flag — δεν χρειάζεται νέο flag για emoji. Το `chatMedia` flag θα προστεθεί στη Φάση 3 (Photo sharing).

#### 5.3.4 Error handling (AppMessenger + ErrorMessages)

```dart
// Αν emoji_picker_flutter έχει κάποιο σπάνιο runtime error (απίθανο):
DebugConfig.error('ChatInputBar: emoji error', data: e, exception: s);
AppMessenger.showError(context, ErrorMessages.get('chat/emoji-error', greek));
```

Ακολουθεί το υπάρχον pattern — `AppMessenger.showError()` + `ErrorMessages.get()` (ποτέ raw ScaffoldMessenger ή AlertDialog).

#### 5.3.5 State management (ConsumerStatefulWidget)

```dart
class ChatInputBar extends ConsumerStatefulWidget {
  final String chatId;
  final bool isGroupChat;
  const ChatInputBar({super.key, required this.chatId, this.isGroupChat = false});
...
class _ChatInputBarState extends ConsumerState<ChatInputBar> {
  ...
  // mounted checks σε κάθε async operation
  if (!mounted) return;
}
```
...
Container(
  ...
  // ... emoji picker uses Material 3 default theme (συμβατό)
)
```

### 5.4 Υλοποίηση — chat_input_bar.dart (~180 γραμμές)
...
class _ChatInputBarState extends ConsumerState<ChatInputBar> {
  ...
  void _onEmojiSelected(Emoji emoji) {
    ...
    if (pos < 0 || pos > text.length) {
      // Αν δεν υπάρχει selection (π.χ. χωρίς focus), append
...
### 5.5 Τροποποίηση — chat_screen.dart

```diff
+ import '../widgets/chat_input_bar.dart';

 // Στο build(), αντικατάσταση:
...
### 5.6 Edge Cases & Θωράκιση

| Σενάριο | Προστασία |
|---------|-----------|
| **Keyboard + emoji picker overlap** | Column με 250px SizedBox — το picker είναι πάντα κάτω από το text field |
| **Landscape mode** | `EmojiPicker` config: `clipBehavior: Clip.none` — αποτρέπει overflow clipping |
| **Insert emoji at cursor (όχι append)** | `_onEmojiSelected()`: `text.substring(0, pos)` + emoji + `text.substring(pos)` |
| **Rapid emoji taps** | Cursor position επαναϋπολογίζεται κάθε φορά μέσω `_textCtrl` — thread-safe |
| **Cursor not visible (no focus)** | `baseOffset` < 0 → emoji appended στο τέλος |
| **canComm = false** | Δεν εμφανίζεται emoji button (το Row έχει μόνο verified UI) |
| **_isLoading = true** | Δεν εμφανίζεται emoji button (loading state) |
| **Hot reload** | `_emojiPickerVisible` reset → picker εξαφανίζεται (αποδεκτό behavior) |
| **App background → resume** | Emoji picker κλείνει αυτόματα από platform |
| **Route pop** | autoDispose → `TextEditingController.dispose()` |
| **TextField με πολύ κείμενο** | Emoji εισάγεται στη σωστή θέση — substring operations |
| **iOS/Android keyboard** | `emoji_picker_flutter` handles natively |

### 5.7 Flutter Lifecycle Analysis
...
| **Route pop** | `ChatInputBar.dispose()` → `TextEditingController.dispose()` ✅ |
...
| 5 | `flutter run` + device test: emoji toggle, insert, send | Λειτουργικό ✅ |

### 5.10 Σύνοψη Flags, Keys & Guards
